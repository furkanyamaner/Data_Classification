...
# Read data
dFrame = pd.read_excel("/Users/furkanyamaner/Desktop/adsız klasör 2/Data_Classification/Data_processed.xlsx")
...
def draw_combined_roc(models, X, y, categorical_columns):
    """
    Tüm modelleri ve kategorik özellikleri dikkate alarak tek bir ROC eğrisini çizer.
    """
    # Kategorik özelliklerin kombinasyonları üzerinde gruplama yapma
    grouped_data = X[categorical_columns]  # Sadece kategorik özellikler
    X_group = X
    y_group = y
    
    plt.figure(figsize=(10, 6))  # Tek bir ROC grafiği için boyut
    for model in models:
                ...
                if y_prob is not None:
                    if len(np.unique(y_group)) > 2:  # Çok sınıflı
                        ...
                        for i in range(y_test_binarized.shape[1]):
                            ...
                            roc_auc = auc(fpr, tpr)
                            plt.plot(fpr, tpr, label=f'{model_name} Class {i} Fold {fold_idx} AUC: {roc_auc:.2f}')
                    else:  # İkili sınıf
                        fpr, tpr, _ = roc_curve(y_test, y_prob[:, 1])
                        roc_auc = auc(fpr, tpr)
                        plt.plot(fpr, tpr, label=f'{model_name} Fold {fold_idx} AUC: {roc_auc:.2f}')

        except Exception as e:
            print(f"ROC çizdirilemedi ({model_name} için {categorical_columns}): {e}")

    # ROC Grafiği Özellikleri
    plt.title(f'Combined ROC Curve for All Models with Categorical Attributes')
...
draw_combined_roc(models, X, y, categorical_columns)
